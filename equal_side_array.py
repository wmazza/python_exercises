...
def find_even_index(arr):
    ...
    for i in range(len(arr)):
        temp_subarray1 = arr[0:i]  
        temp_subarray2 = arr[i+1:]
        print(temp_subarray1) 
        print(temp_subarray2)
        print('SUM: ' + str(sum(temp_subarray1)) + ' - ' + str(sum(temp_subarray2)))
        if sum(temp_subarray1) == sum(temp_subarray2):
            return i
